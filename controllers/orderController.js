...
exports.getCompletedOrders = async (req, res) => {
    try {
      const orders = await Order.find({ status: 'completed' }).sort({ createdAt: -1 }).populate('menuItemIds');
      res.status(200).json(orders);
    } catch (err) {
      res.status(500).json({ message: 'Could not fetch completed orders', error: err.message });
    }
  };
...
exports.getPendingOrders = async (req, res) => {
    try {
      ...
      res.status(500).json({ message: 'Could not fetch pending orders', error: err.message });
    }
  };
